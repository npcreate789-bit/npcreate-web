...
export default async function EditCampaignPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [campaign, products] = await Promise.all([getCampaign(id), getStoreProducts()])
    ...
    <div className="min-h-screen bg-[#0A0808] pt-10 pb-16">
      <div className="max-w-xl mx-auto px-4 sm:px-6 space-y-5">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-display font-bold text-white text-xl">แก้ไขแคมเปญ</h1>
        </div>
        <CampaignForm campaign={campaign} products={products} />
      </div>
    </div>
  )
}
