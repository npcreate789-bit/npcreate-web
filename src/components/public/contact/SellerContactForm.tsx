  ...
  return (
    <div className="bg-[#1C0D0D] border border-white/5 rounded-2xl p-6 sm:p-8 space-y-5">
      <div className="text-center space-y-1.5">
        ...
        <p className="text-slate-500 text-sm">เพื่อให้ทีมงานติดต่อกลับได้อย่างถูกต้องและปลอดภัย</p>
      </div>

      <a
        href="/api/auth/line?mode=member&returnTo=/contact"
...
export function SellerContactForm({ hasSubmitted, lineSession, isMember, lineOaHref, prefill }: Props) {
      ...
      <div className="bg-[#1C0D0D] border border-white/5 rounded-2xl p-8 sm:p-10 text-center space-y-5">
        <div>
          ...
          <p className="text-slate-400 text-sm leading-relaxed max-w-sm mx-auto">
            ทีมงานได้รับข้อมูลและจะโทรกลับหรือทัก LINE ภายใน 1 ชั่วโมง
  ...
  if (!isMember) return <LoginPrompt />
  ...
  return (
    ...
    >
      ...
      <Field label="ยอดขายปัจจุบัน (GMV/เดือน)" error={errors.monthly_gmv?.message} required>
        <select {...register("monthly_gmv")} className={inputCls(!!errors.monthly_gmv)}>
          <option value="">— เลือกยอดขายปัจจุบัน —</option>
          {gmvOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
        </select>
      </Field>
      ...
      <Field label="ข้อความเพิ่มเติม (ถ้ามี)">
        <textarea
          {...register("message")}
          placeholder="เล่าให้เราฟังว่าตอนนี้ shop เป็นยังไง หรือมีคำถามอะไร"
          rows={4}
